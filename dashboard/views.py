...
def my_s3_upload(file, object_name=None):
    bucket_name = "s3-cpp-x23300841"
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 'image' in request.FILES:
                ...
                # ---- Save image locally in MEDIA ----
                product.image.save(image_file.name, image_file, save=False)

                # ---- Upload to S3 ----
                upload_success = my_s3_upload(image_file, image_file.name)

                if upload_success:
                    # Optional (store public S3 URL in a separate field)
                    product.image_url = f"https://s3-cpp-x23300841.s3.amazonaws.com/{image_file.name}"
                else:
                    messages.error(request, "Image upload to S3 failed!")
                    return redirect('dashboard:add_product')
